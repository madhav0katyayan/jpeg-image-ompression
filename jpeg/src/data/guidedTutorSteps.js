  ...
  return [
    {
      ...
      text: "After the zig-zag scan, the AC values often contain long runs of zeros, especially near the end. Run-Length Encoding (RLE) counts consecutive repeated values and writes each run as (value, number of repetitions).",
      target: null,
    },
    {
      title: "Run Run-Length Encoding",
      text: "This button counts each run of repeated values one by one and fills the table.",
      target: ".step9ControlBar",
      placement: "bottom",
      requiresAction: (ctx) => !!ctx.isStep10Complete,
      actionMessage: "Please run \"Run Run-Length Encoding\" before continuing.",
    },
    {
      title: "AC Values From Step 8",
      text: "These are the 63 AC values from the zig-zag scan. Same values next to each other form one run. Click any cell to highlight its run.",
      target: ".step9Card",
      placement: "right",
    },
    {
      title: "Encoded Stream",
      text: "Each pair is one run written as (value, count) — this value repeated this many times in a row. These pairs go on to Step 10 (Huffman Encoding).",
